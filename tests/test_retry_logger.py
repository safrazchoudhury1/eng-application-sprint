...
def test_upload_with_retries_success_when_always_succeeds():
    ...
    try:
        ...
        assert (
            retry_logger.upload_with_retries("dummy", max_retries=2, base_delay=0.0)
            is True
        )
    finally:
...
def test_upload_with_retries_gives_up(monkeypatch):
    monkeypatch.setattr(retry_logger, "attempt_upload", lambda x: False)
    assert (
        retry_logger.upload_with_retries("dummy", max_retries=2, base_delay=0.0)
        is False
    )
