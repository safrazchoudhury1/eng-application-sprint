...
def run_jobs(num_jobs: int, max_retries: int, base_delay: float) -> None:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(
                upload_with_retries, job_id, max_retries, base_delay
            ): job_id
...
def parse_args(argv: Optional[list] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Simulate uploads with retries and backoff."
    )
    parser.add_argument(
        "--jobs", type=int, default=1, help="Number of jobs to run concurrently"
    )
    parser.add_argument(
        "--max-retries", type=int, default=5, help="Maximum retries per job"
    )
    parser.add_argument(
        "--base-delay",
        type=float,
        default=0.5,
        help="Initial delay in seconds before retrying",
    )
